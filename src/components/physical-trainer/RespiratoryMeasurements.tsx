...
import { useRespiratoryMeasurements } from '../../hooks/usePhysicalTests';
import { getCategoryByDOBToday } from '../../utils/categoryUtils';

interface RespiratoryRow {
  id: number;
  athleteId?: string;
  name: string;
...
interface RespiratoryMeasurementsProps {
  clubId: string;
}

const RespiratoryMeasurements: React.FC<RespiratoryMeasurementsProps> = ({ clubId }) => {
  const { athletes: clubAthletes } = useClubAthletes(clubId);
  const { data: firestoreData, saveData } = useRespiratoryMeasurements(clubId);
  const [rows, setRows] = useState<RespiratoryRow[]>([]);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (Array.isArray(firestoreData) && firestoreData.length > 0) {
      setRows(firestoreData as RespiratoryRow[]);
    }
  }, [firestoreData]);
  ...
  useEffect(() => {
    ...
    setRows(prev => {
      let updated: RespiratoryRow[] = [...prev];
  ...
  const updateRow = (id: number, field: keyof RespiratoryRow, value: string) => {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await saveData(rows);
      alert('تم حفظ قياسات الجهاز التنفسي بنجاح');
    } catch (err) {
      console.error('Error saving respiratory measurements:', err);
          ...
          <Table bordered className="cardio-table coach-table" dir="rtl">
            <thead>
              <tr className="table-primary">
                ...
                <th className="text-center align-middle">VO2MAX</th>
                <th className="text-center align-middle">عتبة اللاكتات</th>
                <th className="text-center align-middle">VT1-VT2</th>
                ...
                <tr key={row.id}>
                  ...
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      size="sm"
                      value={row.vo2max}
                      onChange={(e) => updateRow(row.id, 'vo2max', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
                      type="text"
                      size="sm"
                      value={row.lactateThreshold}
                      onChange={(e) => updateRow(row.id, 'lactateThreshold', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      size="sm"
                      value={row.vt}
                      onChange={(e) => updateRow(row.id, 'vt', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
...
export default RespiratoryMeasurements;
