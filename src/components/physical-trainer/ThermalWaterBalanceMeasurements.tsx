...
import { useThermalWaterBalanceMeasurements } from '../../hooks/usePhysicalTests';
import { getCategoryByDOBToday } from '../../utils/categoryUtils';

interface ThermalWaterRow {
  id: number;
  athleteId?: string;
  name: string;
  category: string; // الصنف
  group: string; // الفئة
  bodyTemp: string; // درجة حرارة الجسم
  sweatRate: string; // معدل التعرق وفقدان السوائل
  notes: string;
}

interface ThermalWaterBalanceMeasurementsProps {
  clubId: string;
}

const ThermalWaterBalanceMeasurements: React.FC<ThermalWaterBalanceMeasurementsProps> = ({ clubId }) => {
  const { athletes: clubAthletes } = useClubAthletes(clubId);
  const { data: firestoreData, saveData } = useThermalWaterBalanceMeasurements(clubId);
  const [rows, setRows] = useState<ThermalWaterRow[]>([]);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (Array.isArray(firestoreData) && firestoreData.length > 0) {
      setRows(firestoreData as ThermalWaterRow[]);
    }
  }, [firestoreData]);
  ...
  useEffect(() => {
    ...
    setRows(prev => {
      let updated: ThermalWaterRow[] = [...prev];
  ...
  const updateRow = (id: number, field: keyof ThermalWaterRow, value: string) => {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await saveData(rows);
      alert('تم حفظ قياسات الحرارة والتوازن المائي بنجاح');
    } catch (err) {
      console.error('Error saving thermal/water balance measurements:', err);
          ...
          <Table bordered className="cardio-table coach-table" dir="rtl">
            <thead>
              <tr className="table-primary">
                ...
                <th className="text-center align-middle">درجة حرارة الجسم</th>
                <th className="text-center align-middle">معدل التعرق وفقدان السوائل</th>
                ...
                <tr key={row.id}>
                  ...
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      size="sm"
                      value={row.bodyTemp}
                      onChange={(e) => updateRow(row.id, 'bodyTemp', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      size="sm"
                      value={row.sweatRate}
                      onChange={(e) => updateRow(row.id, 'sweatRate', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
...
export default ThermalWaterBalanceMeasurements;
