...
import { useHormonalMeasurements } from '../../hooks/usePhysicalTests';
import { getCategoryByDOBToday } from '../../utils/categoryUtils';

interface HormonalRow {
  id: number;
  athleteId?: string;
  name: string;
...
interface HormonalMeasurementsProps {
  clubId: string;
}

const HormonalMeasurements: React.FC<HormonalMeasurementsProps> = ({ clubId }) => {
  const { athletes: clubAthletes } = useClubAthletes(clubId);
  const { data: firestoreData, saveData } = useHormonalMeasurements(clubId);
  const [rows, setRows] = useState<HormonalRow[]>([]);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (Array.isArray(firestoreData) && firestoreData.length > 0) {
      setRows(firestoreData as HormonalRow[]);
    }
  }, [firestoreData]);
  ...
  const updateRow = (id: number, field: keyof HormonalRow, value: string) => {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await saveData(rows);
      alert('تم حفظ قياسات الهرمونات بنجاح');
    } catch (err) {
      console.error('Error saving hormonal measurements:', err);
          ...
          <Table bordered className="cardio-table coach-table" dir="rtl">
            <thead>
              <tr className="table-primary">
                ...
                <th className="text-center align-middle">الكورتيزول</th>
                <th className="text-center align-middle">التستوستيرون</th>
                ...
                <tr key={row.id}>
                  ...
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      size="sm"
                      value={row.cortisol}
                      onChange={(e) => updateRow(row.id, 'cortisol', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
                      ...
                      onChange={(e) => updateRow(row.id, 'testosterone', e.target.value)}
                    />
                  </td>
                  <td className="text-center align-middle">
                    <Form.Control
...
export default HormonalMeasurements;
